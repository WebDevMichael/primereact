...
export class TreeTable extends Component {
  ...
  getCalculatedSortOrder(currentOrder) {
    return this.props.removableSort
      ? this.props.defaultSortOrder === currentOrder
        ? currentOrder * -1
        : 0
      : currentOrder * -1;
  }
  ...
  sortMultiple(data) {
    ...
    if (multiSortMeta) return this.sortMultipleNodes(data, multiSortMeta);
    else return data;
  }
    ...
    if (this.draggedColumnEl) {
      ...
      if (allowDrop) {
        ...
        let isSameColumn = (col1, col2) =>
          col1.props.columnKey || col2.props.columnKey
    ...
    for (let node of value) {
      ...
      for (let j = 0; j < columns.length; j++) {
        ...
        if (filterMeta) {
          ...
          filterConstraint =
            filterMatchMode === "custom" ? col.props.filterFunction : FilterService.filters[filterMatchMode];
          options = {
            rowData: node,
            filters,
            props: this.props,
  ...
  processValue() {
    ...
    if (this.props.value && this.props.mongoDbData)
      throw new Error("Both props 'props.value' and 'props.mongoDbData' cannot both be passed");
    ...
    if (!this.props.lazy) {
      if (data && data.length) {
        if (this.getSortField() || this.getMultiSortMeta()) {
          if (this.props.sortMode === "single") data = this.sortSingle(data);
          else if (this.props.sortMode === "multiple") data = this.sortMultiple(data);
        }
  ...
  createTableFooter(columns, columnGroup) {
    return <TreeTableFooter columns={columns} columnGroup={columnGroup} />;
  }
  ...
  renderTable(value) {
    if (this.props.scrollable) return this.renderScrollableTable(value);
    else return this.renderRegularTable(value);
  }
  ...
  render() {
    ...
    const resizeHelper = this.props.resizableColumns && (
      <div
        ref={(el) => {
    ...
    const reorderIndicatorUp = this.props.reorderableColumns && (
      <span
    ...
    const reorderIndicatorDown = this.props.reorderableColumns && (
      <span
