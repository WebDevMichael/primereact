...
export class TreeTableBodyCell extends Component {
  ...
  onClick() {
    if (
      this.props.editor &&
      !this.state.editing &&
      (this.props.selectOnEdit || (!this.props.selectOnEdit && this.props.selected))
    ) {
      this.selfClick = true;
  ...
  render() {
    ...
    if (this.state && this.state.editing) {
      if (this.props.editor)
        content = ObjectUtils.getJSXElement(this.props.editor, {
      ...
      if (this.props.body)
        content = ObjectUtils.getJSXElement(this.props.body, this.props.node, {
          field: this.props.field,
          rowIndex: this.props.rowIndex,
          props: this.props,
        });
    ...
    /* eslint-disable */
    const editorKeyHelper = this.props.editor && (
      <a
        tabIndex={0}
        ref={(el) => {
          this.keyHelper = el;
