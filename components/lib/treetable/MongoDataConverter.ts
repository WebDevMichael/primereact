...
function getChildren(
  ...
  if (isArray(mongoQueryResultDocument)) {
    // @ts-ignore
      const subRows = mongoQueryResultDocument.reduce<TreeNode[]>(
      (acc2, mongoRowField: string, index) => {
        ...
        if (
          valueType === MONGO_VALUE_TYPE.OBJECT &&
          isObject(mongoFieldValue)
        ) {
          newSubRow.children = getChildren(
            mongoFieldValue as Record<string, unknown>,
            level++
          );
        }
    ...
    const subRows = Object.keys(mongoQueryResultDocument).reduce<TreeNode[]>(
      (acc2, key: string) => {
...
export function mongoDataConverter(
  mongoResult: MongoResultDocument[]
): TreeNode[] {
  const output = mongoResult.reduce<TreeNode[]>(
    (acc, mongoQueryResultDocument: MongoRow, index) => {
      acc.push({
        key: `${mongoQueryResultDocument._id.toString()}`,
        data: {
          key: `(${
            index + 1
          }) { _id: ${mongoQueryResultDocument._id.toString()} }`,
